...
- **Azure OpenAI Service** (`sc500-lab3c-ai-{instanceId}`)
  - gpt-4o-mini model deployment with 10 TPM capacity
...
3. Confirm **gpt-4o-mini** is deployed and shows "Succeeded" status
...
5. Navigate to **Models + endpoints** and confirm **gpt-4o-mini** is visible
6. Navigate to **Safety + security** > **Content filters** and confirm no custom filter is assigned to gpt-4o-mini (should use "Default" or "None")
...
6. **Apply guardrail to deployment** - Assign custom filter to gpt-4o-mini
...
### Issue: gpt-4o-mini deployment fails
...
- **Model version:** The template deploys `gpt-4o-mini` version `2024-07-18`. Adjust `modelVersion` parameter if a different version is required.
- **Quota requirements:** Ensure the subscription has sufficient Azure OpenAI quota (minimum 10 TPM for gpt-4o-mini).
